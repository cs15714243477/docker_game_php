window.rootPath = (function(src) {
	...
	if(location.pathname=='/player/playerInfo'){
		//console.log(document.scripts[document.scripts.length - 3]);
		src = document.scripts[document.scripts.length - 3].src;
	}else{
		src = document.scripts[document.scripts.length - 1].src;
	}
	//console.log(src);
	src = src.substring(0, src.lastIndexOf("/") + 1);
	if(src.indexOf('pear') == -1) src += 'component/pear/';
	return src;
})();
